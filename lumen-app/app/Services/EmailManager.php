...
class EmailManager extends Service 
{
	...
	function __construct($content_type)
	{
		...
		app()->configure('emailmanager');

		$this->_namespace = config('emailmanager.manager');

		$content_type = ucwords(preg_replace('/[^a-z0-9]/', '', strtolower($content_type)));

		$class = 'App\\Services\\EmailManagers\\'.$this->_namespace.'\\'.$content_type;
